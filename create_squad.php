...
<?php
    ...
    if(isset($_GET['squad']) && isset($_GET['password1']) && isset($_GET['password2']) && isset($_GET['image'])){
        newUser($_GET['username'], $_GET['squad'], $_GET['password1'], $_GET['password2'], $_GET['image']);
    }
    
    function newUser($username, $squad, $pass1, $pass2, $pic){
        $host = "127.0.0.1";
        ...
        if($pass1 == $pass2){
			...
			$currentID = $currentID + 1;
		      $sql = "INSERT INTO squads (id, squad, status, members, password, image) VALUES (".$currentID.", '".$squad."', 'private', '".$_GET['username']."', '".$pass1."', '".$pic."')";
    		  ...
    		  while ($row = mysqli_fetch_assoc($result)) {
                  if ($row['username'] == $username){
                    $sql = "UPDATE users SET squads='".$row['squads'].$squad."+' WHERE username='".$username."'";
                    if ($connection->query($sql) === TRUE) {
    		          echo "<script> window.location='https://squad-sneharams.c9users.io/squads.php?username=".$_GET['username']."'</script>";
      				  exit();
    		        } else {
      ...
      <aside>
          <div id="sidebar"  class="nav-collapse ">
              ...
              <ul class="sidebar-menu" id="nav-accordion">
                  ...
                  <li class="sub-menu">
                      <a class="active" href="create_squad.php?username=<?php
                                                                  echo $_GET['username'];
                                                                  ?>">
                          <i class="fa fa-dashboard"></i>
          ...
          <section class="wrapper">
       	  ...
       	  <div class="row">
       	     <div class="col-lg-9 main-chart">
    		      ...
    		      <form  method="get">
    		        <h2 class="form-login-heading">Create Squad</h2>
    		        <br><br>
    		        <div>
                        ...
                        <br>
                        <input name="image" type="text" class="form-control" placeholder="Squad Pic" autofocus>
                        <br><br>
    		            <button class="btn btn-theme btn-block" href="index.php" type="submit"> JOIN SQUAD</button>
    		            <hr>
    		        </div>
		          </form>	  	
	  	        </div>
